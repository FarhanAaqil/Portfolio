...
renderer.setPixelRatio(Math.min(devicePixelRatio,1.7));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.15;host.append(renderer.domElement);host.tabIndex=0;
...
const water=mesh(new T.PlaneGeometry(3500,3500,200,200),waterMat,0,-.1,-500);water.rotation.x=-Math.PI/2;
...
function paint(now){
 ...
 if(inspectShip){lookAt.copy(ship.position).add(new T.Vector3(0,3.3,0));camPos.copy(ship.position).add(new T.Vector3(16,12,22).applyAxisAngle(new T.Vector3(0,1,0),ship.rotation.y));}
 ...
 if(exploring)camera.clearViewOffset();else camera.setViewOffset(innerWidth,innerHeight,-innerWidth*.23*(window.voyageComposition??1),0,innerWidth,innerHeight);
