...
const crew=createStrawHats(ship);let crewFocus=-1;let autoFrame=0;const pointerRay=new T.Raycaster();
...
if(exploring){const offset=camPos.clone().sub(lookAt);const sph=new T.Spherical().setFromVector3(offset);sph.theta+=yaw;sph.phi=T.MathUtils.clamp(sph.phi+pitch,.2,1.45);camPos.copy(lookAt).add(offset.setFromSpherical(sph));}else if(!mobile()&&!document.body.classList.contains('world-first')){lookAt.x-=8;}
if(crewFocus>=0){const person=crew[crewFocus];ship.updateWorldMatrix(true,true);const target=new T.Vector3();person.getWorldPosition(target);target.y+=.7*person.scale.y;lookAt.copy(target);const offset=new T.Vector3((person.position.x<0?-1:1)*3.1*Math.cos(yaw),2.1+pitch,1+Math.sin(yaw)*2.5).applyAxisAngle(new T.Vector3(0,1,0),ship.rotation.y);camPos.copy(target).add(offset);}
camera.position.copy(camPos);camera.lookAt(lookAt);sun.target.position.copy(lookAt);sun.position.copy(lookAt).add(new T.Vector3(-25,42,25));if(moving){crew.forEach((person,i)=>{person.position.y=person.userData.baseY+Math.sin(now*.0012+i)*.008;});waterMat.uniforms.time.value=now*.0006;blades.rotation.z=now*.00015;falls.forEach((f,i)=>{f.scale.x=.9+Math.sin(now*.003+i)*.1;});}renderer.render(scene,camera);document.querySelector('#route-ship').style.left=`${t*100}%`;document.querySelector('#position').textContent=blend>.1?'SAILING TO '+next.label:current.label;document.querySelector('#destination').value=current.id;document.querySelectorAll('[data-sail]').forEach(b=>b.classList.toggle('active',b.dataset.sail===current.id));document.body.dataset.destination=current.id;if(moving||Math.abs(progress-targetProgress)>.001)requestFrame();}
...
window.voyageDiagnostics=()=>({progress,destination:document.body.dataset.destination,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,crew:crew.map(p=>p.name),crewFocus,exploring,inspectShip,yaw,pitch,moving});
