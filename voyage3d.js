...
renderer.setPixelRatio(Math.min(devicePixelRatio,mobile()?1.25:1.7));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;host.append(renderer.domElement);host.tabIndex=0;
...
scene.add(new T.HemisphereLight('#fff6dc','#32777c',1.8));const sun=new T.DirectionalLight('#ffe4ac',2.7);sun.position.set(-25,50,30);scene.add(sun);
const mats={};const mat=(color)=>mats[color]??(mats[color]=new T.MeshStandardMaterial({color,roughness:.85,flatShading:false}));
...
const water=mesh(new T.PlaneGeometry(1000,1000,mobile()?90:150,mobile()?90:150),waterMat,140,-.1,-45);water.rotation.x=-Math.PI/2;
...
const city=new T.Group();city.name='Water 7';city.position.set(77,0,-37);scene.add(city);
...
for(let i=0;i<18;i++){const g=new T.Group();g.position.set(-65+rand()*205,24+rand()*16,-90+rand()*85);for(let j=0;j<4;j++)sphere(j*2.5,rand(),0,2.7,'#f2f0dc',g,1.5,.65,1);scene.add(g)}for(let i=0;i<8;i++)sphere(-70+i*30,-1,-90-rand()*25,6+rand()*7,'#7daf9b',scene,1,.8,1);
...
const reverse=new T.Group();reverse.position.set(36,0,-27);scene.add(reverse);
...
const sky=new T.Group();sky.position.set(146,0,-58);scene.add(sky);
...
const egg=new T.Group();egg.position.set(220,0,-74);scene.add(egg);cyl(0,.2,0,13,11,2,'#a7bcbc',egg,40);cyl(0,1.4,0,12.8,13,.6,'#f1ede0',egg,40);
...
const haven=new T.Group();haven.position.set(292,0,-93);scene.add(haven);cyl(0,.2,0,9,8,2,'#dbbd82',haven,32);cyl(0,1.3,0,8.5,9,.4,'#b3b880',haven,32);cyl(-3,5,0,1,1.5,7,'#f0ddae',haven,16);cyl(-3,8.7,0,1.4,1.4,.5,'#ba6246',haven,16);cyl(-3,9.5,0,.85,.85,1.2,'#f3c662',haven,16);mesh(new T.ConeGeometry(1.5,1,16),'#ac5a43',-3,10.5,0,haven);
...
const stops=[
{id:'foosha',label:'FOOSHA VILLAGE',at:new T.Vector3(0,2,1),boat:new T.Vector3(6,.3,17),eye:new T.Vector3(27,23,37)},
{id:'about',label:'REVERSE MOUNTAIN',at:new T.Vector3(36,5,-27),boat:new T.Vector3(36,.3,-10),eye:new T.Vector3(65,29,12)},
{id:'water7',label:'WATER 7',at:new T.Vector3(77,5,-37),boat:new T.Vector3(70,.3,-14),eye:new T.Vector3(111,30,4)},
{id:'projects',label:'SKYPIEA',at:new T.Vector3(146,10,-58),boat:new T.Vector3(142,.3,-40),eye:new T.Vector3(178,32,-16)},
{id:'egghead',label:'EGGHEAD',at:new T.Vector3(220,6,-74),boat:new T.Vector3(211,.3,-53),eye:new T.Vector3(254,29,-31)},
{id:'contact',label:'THE NEW WORLD',at:new T.Vector3(292,3,-93),boat:new T.Vector3(290,.3,-77),eye:new T.Vector3(319,24,-56)}
];const chapters=stops.map(s=>document.getElementById(s.id));
...
function readScroll(){targetProgress=T.MathUtils.clamp(scrollY/(document.documentElement.scrollHeight-innerHeight),0,1);requestFrame()}
function paint(now){frame=0;if(document.hidden||document.body.classList.contains('reading-view'))return;const dt=Math.min((now-last)/1000,.05)||.016;last=now;progress=moving?T.MathUtils.damp(progress,targetProgress,5,dt):targetProgress;const t=progress;const sy=t*Math.max(1,document.documentElement.scrollHeight-innerHeight);let index=0;for(let i=0;i<chapters.length;i++){if(sy>=chapters[i].offsetTop-2)index=i;}const current=stops[index],next=stops[Math.min(index+1,stops.length-1)];const length=chapters[index].offsetHeight;const local=T.MathUtils.clamp((sy-chapters[index].offsetTop)/length,0,1);const blend=index===stops.length-1?0:T.MathUtils.smoothstep(local,.72,1);const pos=new T.Vector3().lerpVectors(current.boat,next.boat,blend);ship.position.copy(pos);ship.position.y+=moving?Math.sin(now*.0015)*.12:0;const dir=new T.Vector3().subVectors(next.boat,current.boat);ship.rotation.y=dir.lengthSq()>0?Math.atan2(dir.x,dir.z):.8;ship.rotation.z=moving?Math.sin(now*.001)*.035:0;
camPos.lerpVectors(current.eye,next.eye,blend);lookAt.lerpVectors(current.at,next.at,blend);
...
if(mobile()){camPos.y+=11;camPos.z+=14;lookAt.y+=6;}
...
if(exploring){const offset=camPos.clone().sub(lookAt);const sph=new T.Spherical().setFromVector3(offset);sph.theta+=yaw;sph.phi=T.MathUtils.clamp(sph.phi+pitch,.2,1.45);camPos.copy(lookAt).add(offset.setFromSpherical(sph));}else if(!mobile()){lookAt.x-=8;}
camera.position.copy(camPos);camera.lookAt(lookAt);if(moving){waterMat.uniforms.time.value=now*.0006;blades.rotation.z=now*.00015;falls.forEach((f,i)=>{f.scale.x=.9+Math.sin(now*.003+i)*.1;});}renderer.render(scene,camera);document.querySelector('#route-ship').style.left=`${t*100}%`;document.querySelector('#position').textContent=blend>.1?'SAILING TO '+next.label:current.label;document.querySelector('#destination').value=current.id;document.querySelectorAll('[data-sail]').forEach(b=>b.classList.toggle('active',b.dataset.sail===current.id));document.body.dataset.destination=current.id;if(moving||Math.abs(progress-targetProgress)>.001)requestFrame();}
function requestFrame(){if(!frame)frame=requestAnimationFrame(paint)}
function setExplore(value){exploring=value;document.body.classList.toggle('exploring',value);document.querySelector('#look').setAttribute('aria-pressed',String(value));document.querySelector('#look').textContent=value?'Return to voyage ×':'Look around ⊹';document.querySelector('#explore-help').hidden=!value;if(value)host.focus({preventScroll:true});else{yaw=0;pitch=0;inspectShip=false;}document.querySelector('#inspect-sunny').setAttribute('aria-pressed',String(inspectShip));requestFrame();}
document.querySelector('#inspect-sunny').addEventListener('click',()=>{inspectShip=!inspectShip;yaw=0;pitch=0;setExplore(inspectShip)});
...
window.voyageDiagnostics=()=>({progress,destination:document.body.dataset.destination,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,geometries:renderer.info.memory.geometries,exploring,inspectShip,yaw,pitch,moving});
